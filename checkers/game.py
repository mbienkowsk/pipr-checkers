import pygame
from checkers.constants import (FIELD_SIZE, PIECE_PADDING, GREEN, BROWN,
                                SLEEP_TIME_IN_PVB_GAME, SLEEP_TIME_IN_BVB_GAME,
                                Color)
from checkers.piece_move_board import Board, Piece
...
class Game:
    ...
    def load_images(self):
        '''Load the images from the folder and return a dictionary with them'''
        #  tone down the alpha value of the white pieces so they don't
        #  stand out after the game over animation
        WP = pygame.transform.scale(pygame.image.load(
            'images/white_piece.png').convert_alpha(),
            (FIELD_SIZE - PIECE_PADDING, FIELD_SIZE - PIECE_PADDING))
        WP.set_alpha(200)
        ...
        self.images = {
            'WP': WP,
            'BK': pygame.transform.scale(pygame.image.
                                         load('images/black_king.png').
                                         convert_alpha(),
                                         (FIELD_SIZE - PIECE_PADDING,
                                          FIELD_SIZE - PIECE_PADDING)),
            'WK': WK,
            'BP': pygame.transform.scale(pygame.image.
                                         load('images/black_piece.png').
                                         convert_alpha(),
                                         (FIELD_SIZE - PIECE_PADDING,
                                          FIELD_SIZE - PIECE_PADDING))
        }
    ...
    def draw_board(self):
        '''Display the boardäs current state including fields and pieces'''
        self.draw_fields()
        self.draw_pieces()
    ...
    def draw_fields(self):
        ...
        for field in self.board.one_dimensional_field_list:
            field_rect = pygame.Rect(
                field.x * FIELD_SIZE, field.y * FIELD_SIZE,
        ...
        for field in self.board.one_dimensional_field_list:
            if field.is_taken():
                ...
                piece_rect = piece_surf.get_rect(
                    center=current_piece.location_to_draw())
                self.screen.blit(piece_surf, piece_rect)

    def highlight_field(self, field):
        '''Display a green circle onto a given field indicating that
        a move there is possible'''
        field_surf = pygame.Surface((FIELD_SIZE, FIELD_SIZE))
        field_surf.fill(BROWN)
        square_rect = pygame.Rect(
            field.x * FIELD_SIZE, field.y * FIELD_SIZE, FIELD_SIZE, FIELD_SIZE)
        pygame.draw.circle(field_surf, GREEN, (FIELD_SIZE / 2,
                           FIELD_SIZE / 2), FIELD_SIZE * 0.35)
    ...
    def show_possible_moves(self, piece: 'Piece'):
        ...
        possible_move_squares = [self.board.get_field_by_location(
            move.new_cords) for move in possible_moves]
    ...
    def handle_piece_click(self, clicked_piece):
        ...
        if (clicked_piece.color == self.board.turn
                and self.board.can_piece_move(clicked_piece)):
    ...
    def find_move_by_move_location(location, piece_move_list):
        '''Returns a move from the given list with the new_cords
        param matching the given location'''
        ...
        if self.selected_piece is not None:
            (possible_move_locations,
             possible_moves) = (self.board.
                                feasible_locations_and_moves_for_piece(
                                    self.selected_piece))

            if clicked_field.location in possible_move_locations:
                move_to_make = self.find_move_by_move_location(
                    clicked_field.location, possible_moves)
    ...
    def interpret_clicked_pixel_location(self, location):
        ...
        clicked_field = self.board.get_field_by_location(
            clicked_field_location)
    ...
    def handle_mouse_click(self, click_position):
        ...
        clicked_field, clicked_piece = self.interpret_clicked_pixel_location(
            click_position)
    ...
    def handle_random_bot_move(self):
        ...
        if bot.color == Color.WHITE:
            movable_pieces = [piece for piece in self.board.all_white_pieces(
            ) if self.board.moves_by_colors[Color.WHITE][piece]]
            piece_click_location = bot.click_random_piece(movable_pieces)
        else:
            movable_pieces = [piece for piece in self.board.all_black_pieces(
            ) if self.board.moves_by_colors[Color.BLACK][piece]]
            piece_click_location = bot.click_random_piece(movable_pieces)
        self.handle_mouse_click(piece_click_location)
        moves_for_chosen_piece = (self.board.
                                  feasible_locations_and_moves_for_piece(
                                      self.selected_piece)[1])
        move_click_location = bot.choose_random_possible_move_location(
            moves_for_chosen_piece)
